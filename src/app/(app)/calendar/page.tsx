...
export default async function NoticePage() {
  ...
  // ── 店休日・祝日（当日から）────────────────
  type NoticeRow = { key: string; emoji: string; title: string; subtitle: string; url?: string | null; bg: string; isStaff?: boolean };
  ...
  for (const ev of events) {
    ...
    if (ev.schedule_type === "single" && ev.start_date) {
      ...
      rows.push({ key:`ev-${ev.id}`, emoji: ev.emoji, title: ev.title, subtitle: sub, url: ev.url, bg:"bg-[#f5f0ff]", isStaff: true });
      ...
      for (let i=0; i<=30; i++) {
        ...
        if (d.getUTCDay()===ev.weekly_day) {
          ...
          rows.push({ key:`ev-${ev.id}-${ds}`, emoji: ev.emoji, title: ev.title, subtitle: sub, url: ev.url, bg:"bg-[#f5f0ff]", isStaff: true });
          break;
        }
      }
    }
            ...
            {rows.map((item) => (
              <li key={item.key} className={`flex items-start gap-3 px-4 py-3 rounded-app ${item.bg}`}>
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-black text-base text-navy">
                    {item.isStaff ? (
