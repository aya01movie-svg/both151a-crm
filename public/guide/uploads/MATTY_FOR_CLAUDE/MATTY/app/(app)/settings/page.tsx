...
export default async function SettingsPage() {
  ...
  // v1.2パフォーマンス改善: 互いに依存しないクエリを並行実行する
  // （以前は全て順番にawaitしていたため、通信回数分の待ち時間が積み上がっていた）
  const [tags, staff, auditLogs, events, closedDays, customersForManage] = await Promise.all([
    listTags(),
    isAdmin ? listStaff() : Promise.resolve([]),
    isAdmin ? listAuditLogs(50) : Promise.resolve([]),
    listStoreEvents(),
    listClosedDays(cdFrom.toISOString().slice(0, 10), cdTo.toISOString().slice(0, 10)),
    isAdmin ? fetchCustomersForManage() : Promise.resolve([]),
  ]);
...
async function fetchCustomersForManage() {
  ...
  const { data } = await supabase
    ...
    .limit(500);
  return (data ?? []) as { id: string; display_name: string; kana: string | null; visit_count: number }[];
}
