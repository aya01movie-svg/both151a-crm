...
export default async function CustomerDetailPage({
  ...
  // v1.2パフォーマンス改善: 互いに依存しないデータ取得は並行実行する
  // （profileはReact cache()により共通レイアウトと問い合わせが重複しない）
  const [profile, detail, timelineEvents] = await Promise.all([
    getCurrentProfile(),
          ...
          <ul className="flex flex-col gap-3 max-h-[420px] overflow-y-auto">
            {visits.map((v) => (
              <li key={v.id} className="border-b border-navy/5 pb-3 last:border-0">
                ...
                <div className="flex justify-between items-center text-xs text-navy/50 mt-0.5">
                  <span>
                  ...
                  <span className="flex items-center gap-2">
                    {v.people_count}名
                    {v.role === "primary" && (
                      <Link href={`/visits/${v.id}/edit`} className="text-info underline shrink-0">
                        編集
                      </Link>
                    )}
                  </span>
