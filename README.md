...
4. **PWAとしてホーム画面に追加**
   ...
   - PWAアイコンは猫のマスコット画像（`public/icons/cat-icon-*.png`）を使用しています。差し替える場合はファイル名を新しくし（例: `cat-icon-v2-192.png`）、`manifest.webmanifest`・`layout.tsx`・`Header.tsx`・`SideNav.tsx`・`login/page.tsx`の参照先も合わせて変更してください（同じファイル名のまま中身だけ差し替えると、Android端末に古い画像がキャッシュされたまま残ることがあります）

### ⚠️ Android PWAの画面向きが更新されない場合

`manifest.webmanifest`の`orientation`を変更しても、**すでにホーム画面に追加（インストール）済みのPWA**では
反映されないことがあります。これはアプリの不具合ではなく、Androidの仕様によるものです。

**原因**：AndroidはPWAをホーム画面に追加する際、その時点のmanifest内容をもとに
「WebAPK」と呼ばれる実行体をその場で生成します。サーバー側でmanifest.webmanifestを
更新しても、すでに生成済みのWebAPKは自動的には作り直されません（Chromeが数日おきに
バックグラウンドで更新チェックを行いますが、即時ではありません）。

**対処方法（スタッフの端末ごとに1回だけ実施）**：
1. ホーム画面のアイコンを長押しして「アンインストール」（削除）する
2. Chromeの設定 → サイト設定 → 対象サイト → 「消去とリセット」でサイトデータを削除する（省略可）
3. ブラウザで対象URLを開き直し、再度「ホーム画面に追加」を行う

これにより最新のmanifest.webmanifest（`orientation: "any"` = 画面回転に追従）が
反映されたWebAPKが作り直されます。なお、今後の運用でmanifestを変更する頻度は
高くないため、通常はこの対応は初回のみで問題ありません。
