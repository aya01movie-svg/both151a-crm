...
かな検索（ひらがな/カタカナ横断検索）は`customers.kana`（読み方）が
...
## 7. 本番開始前のテストデータ削除（重要・実行は1回だけ）

試作・動作確認中に入力したテストデータが残っている場合、本番運用を始める前に
`optional_reset_test_data_before_production.sql` を実行してまっさらな状態にできます。

**⚠️ 完全削除であり元に戻せません。本番開始前の1回だけ実行してください。**
削除されるもの・残るものの詳細はファイル内のコメントを参照してください
（スタッフアカウント・権限・テーブル構造・DB関数・マイグレーション履歴は残ります）。

誤操作防止のため、ファイル冒頭に実行を止めるブロックが入っています。
内容を確認したうえで、そのブロックを削除してから実行してください。
